...
file_path = 'D:/Product duplication/food.xlsx'
...
# Step 10: Expectation-Maximization (Gaussian Mixture Model)
gmm = GaussianMixture(n_components=2, random_state=42)
gmm.fit(X_train)
predictions_gmm = gmm.predict(X_test)
accuracy_gmm = accuracy_score(y_test, predictions_gmm)
print(f'EM (GMM) Accuracy: {accuracy_gmm}')

# Step 11: Convolutional Neural Network (CNN)
...
model_cnn.fit(X_train_img, y_train_img, epochs=10, batch_size=32)
accuracy_cnn = model_cnn.evaluate(X_test_img, y_test_img)[1]
print(f'CNN Accuracy: {accuracy_cnn}')

# Step 12: VGG16 with local weights
vgg_weights_path = 'F:/Product duplication/vgg16_weights_tf_dim_ordering_tf_kernels_notop.h5'
...
model_vgg.fit(X_train_img, y_train_img, epochs=10, batch_size=32)
